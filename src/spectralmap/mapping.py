...
def _normalize_mode(mode: str | None) -> str:
    ...
    if mode_norm in {"ecl", "eclipse"}:
        return "eclipse"
    if mode_norm in {"dop", "doppler", "doppler-imaging"}:
        return "doppler"
    raise ValueError("mode must be one of {'rotational', 'eclipse', 'doppler'}")
...
class Map:
    """Shared base class for rotational/eclipsed/doppler map inference."""
...
class DopplerMap(Map):
    ...
    def solve_posterior(
        ...
        **kwargs,
        ...
        if lambda_fix is not None:
            raise ValueError("lambda_fix regularization is not used by DopplerMap.solve_posterior().")
        ...
        return self._finalize_doppler_solution(
            ...
            flux_err=flux_err,
            solve_result=solve_result,
            solver_used=solve_kwargs["solver"],
            mode="default",
        )

    def solve_unknown_spectrum_baseline(
        self,
        y: np.ndarray,
        sigma_y: np.ndarray | float | None = None,
        theta: np.ndarray | None = None,
    ...
    def plot_spectra(self, theta: np.ndarray | None = None, normalize: bool | None = None):
        if self.flux is None:
            raise ValueError("No fitted spectral time series is available. Run solve_posterior() first.")
        ...
        model_flux = self._eval_to_numpy(self.map.flux(theta=theta, normalize=normalize))

        import matplotlib.pyplot as plt

        fig, axes = plt.subplots(1, 2, figsize=(9, 3), dpi=150, constrained_layout=True)
        im0 = axes[0].imshow(self.flux, aspect="auto", origin="lower")
        axes[0].set_title("Observed spectra")
        axes[0].set_xlabel("Wavelength index")
        axes[0].set_ylabel("Epoch index")
        fig.colorbar(im0, ax=axes[0], fraction=0.046)

        im1 = axes[1].imshow(model_flux, aspect="auto", origin="lower")
        axes[1].set_title("Model spectra")
...
def make_map(mode: str = "rotational",  **kwargs) -> Map:
    """Factory for rotational/eclipsed/doppler mapping classes."""
    ...
    elif mode_norm == "doppler":
        return DopplerMap(**kwargs)
    else:
        raise ValueError(f"Unknown mode: {mode}")

def make_maps(mode: str = "rotational", **kwargs) -> Maps:
    """Factory for rotational/eclipsed mapping helper classes."""
    mode_norm = _normalize_mode(mode)
    ...
    if mode_norm == "rotational":
        ...
        raise NotImplementedError("Doppler multi-model utilities are not available. Use make_map(mode='doppler', ...).")
    else:
        raise ValueError(f"Unknown mode: {mode}")